...
@Service
public class PluginPromptService {

	@McpPrompt(name = "plugin", description = "A simple hello world prompt from plugin")
	public McpSchema.GetPromptResult pluginPrompt(
		...
		return new McpSchema.GetPromptResult("Greeting", List.of(new McpSchema.PromptMessage(McpSchema.Role.ASSISTANT,
			new McpSchema.TextContent("Hello, " + name + "! This is the prompt from plugin."))));
	}
